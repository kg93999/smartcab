...
class LearningAgent(Agent):
	...
	if testing:
		...
		#self.epsilon = math.cos(self.alpha*self.x)
		#self.epsilon = 1 / self.x **2
		#self.x = self.x + 1
		#self.x = self.x * 2
	...
	if not self.learning:
		...
		if self.epsilon > random.random():
			...
			maximum_Q = self.get_maxQ(state)
			l = []
			for i in self.valid_actions:
				if self.Q[state][i] == maximum_Q:
					l.append(i)
	...
	if self.learning:
		self.Q[state][action] = self.alpha * reward + self.Q[state][action] * (1 - self.alpha)
        return
